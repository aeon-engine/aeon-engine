...
static const uint8_t *find_chunk(const uint8_t *start, const uint8_t *end, uint32_t chunk_id)
{
    RIFF_SubChunk chunk;
...
sample_buffer_ptr codec_wav::create_from_buffer(base_buffer &buffer)
{
    ...
    if (riff_header.chunk_id != fourcc("RIFF") || riff_header.format != fourcc("WAVE"))
        return nullptr;
    ...
    sample_buffer_ptr result = std::make_unique<sample_buffer>(wave_format.num_channels, wave_format.bits_per_sample,
                                                               wave_format.sample_rate, wave_buffer, wave_buffer_size);
