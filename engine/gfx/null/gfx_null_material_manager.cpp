...
material_ptr gfx_null_material_manager::__load(resources::material_ptr mat)
{
    ...
    m->texture_ = std::dynamic_pointer_cast<gfx_null_texture>(texture_mgr.load(texture_image));

    resources::shader_ptr shader_res = mat->get_shader();
    m->shader_ = std::dynamic_pointer_cast<gfx_null_shader>(shader_mgr.load(shader_res));
