...
TEST(test_render_queue, test_render_queue_sort_by_group)
{
    ...
    for (int i = 0; i <= 100; ++i)
    {
        ...
        scene::sprite_ptr s = std::make_shared<aeon::scene::sprite>(nullptr, nullptr, glm::vec2(group_id, 10), 0);
...
TEST(test_render_queue, test_render_queue_sort_by_group_and_sprite_z_order)
{
    ...
    for (int i = 0; i <= 100; ++i)
    {
        ...
        for (int j = 0; j <= 100; ++j)
        {
            ...
            scene::sprite_ptr s = std::make_shared<aeon::scene::sprite>(nullptr, nullptr, glm::vec2(group_id, zorder), zorder);
            glm::mat4 position;
